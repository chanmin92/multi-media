...
def load_coco_data(data_path='./data', split='train', toy=None):
    if toy == True:
        toy = "toy_"
    else:
        toy =""
    ...
    data['features'] = hickle.load(os.path.join(data_path, '%s%s.features.hkl' %(toy, split)))
    with open(os.path.join(data_path, '%s%s.file.names.pkl' %(toy, split)), 'rb') as f:
        data['file_names'] = pickle.load(f)   
    with open(os.path.join(data_path, '%s%s.captions.pkl' %(toy,split)), 'rb') as f:
        data['captions'] = pickle.load(f)
    with open(os.path.join(data_path, '%s%s.image.idxs.pkl' %(toy, split)), 'rb') as f:
...
def sample_coco_minibatch(data, batch_size):
    ...
    #mask = np.zeros(batch_size, np.int64)
