...
class CaptionGenerator(object):
    ...
    def _get_initial_lstm(self, features):
        # with tf.variable_scope('initial_lstm', reuse=None):
    ...
    def _get_initial_lm(self, features):
        with tf.variable_scope('initial_lstm', reuse=None):
            features_mean = tf.reduce_mean(features, 1)

            w_h = tf.get_variable('w_h', [self.D, 50], initializer=self.weight_initializer)
            b_h = tf.get_variable('b_h', [50], initializer=self.const_initializer)
            h = tf.nn.tanh(tf.matmul(features_mean, w_h) + b_h)
            return h
    ...
    def _batch_norm(self, x, mode='train', name=None):
        with tf.variable_scope('batch_norm', reuse=None):
            bn_layer = tf.contrib.layers.batch_norm(inputs=x,
                                                decay=0.95,
                                                center=True,
                                                scale=True,
    ...
    def build_model(self):
        ...
        loss = 0.0
        alpha_list = []
        # lstm_cell = tf.nn.rnn_cell.BasicLSTMCell(num_units=self.H, initializer=self.weight_initializer)
        lstm_cell = tf.nn.rnn_cell.LSTMCell(self.H,
                                            state_is_tuple=True, initializer=self.weight_initializer)
